...
U-IPTV 是一个现代化的网络电视播放平台，允许用户管理和播放各种IPTV频道。支持频道分类管理、播放历史记录、频道搜索功能，以及本地数据存储能力。
...
- **频道管理** - 支持添加、删除、编辑各类IPTV频道
- **频道分类** - 按新闻、体育、电影、娱乐等分类管理频道
- **播放控制** - 完整的播放器控制功能，包括播放/暂停、音量调节、全屏等
- **频道搜索** - 快速搜索已添加的频道
...
项目当前使用浏览器 localStorage 进行数据存储，无需额外配置。所有频道数据均保存在本地。
...
src/
├── components/          # 可复用的UI组件
│   ├── ChannelList.jsx  # 频道列表组件
│   ├── Header.jsx       # 顶部导航组件
│   ├── Player.jsx       # 播放器组件
│   └── Sidebar.jsx      # 侧边栏组件
├── pages/               # 页面组件
│   └── ChannelManagerPage.jsx # 频道管理页面
├── App.jsx              # 主应用组件
├── index.css            # 全局样式
└── index.jsx            # 应用入口文件
```
...
注意：由于项目使用 localStorage，部署后数据将按浏览器隔离存储。
...
某些依赖可能仍需要较新版本的 Node.js。如果遇到构建错误，建议升级到 Node.js 18.0.0 或更高版本。
