...
U-Tools 是一个单页应用(SPA)，使用 React 构建，数据存储在浏览器的 localStorage 中。这意味着部署非常简单，只需将构建后的静态文件托管到任何静态网站托管服务即可。
...
### 数据持久性
由于 U-Tools 使用浏览器 localStorage 存储数据，部署后：
- 每个用户的浏览器将拥有独立的数据存储
- 数据无法在不同浏览器或设备间共享
- 清除浏览器数据将导致收藏和历史记录丢失

### 路由配置
U-Tools 使用 React Router 的 hash 模式（URL 中包含 `#`），这使得应用可以作为静态网站运行而无需后端路由支持。
